...
char *base64decode (const void *b64_decode_this, int decode_this_many_bytes){
    ...
    nextoutlen=decoded_byte_index+1;
    BIO_free_all(b64_bio);  //Destroys all BIOs in chain, starting with b64 (i.e. the 1st one).
...
unsigned char* encrypt(unsigned char* message, char* filename) {
  ...
  while((c=fgetc(fp))!=EOF){
	...
	memset(encrypted_message_full, 0, sizeof(encrypted_message_full));

	//printf("adding on %s", encrypted_message_full);

  	memmove(encrypted_message_full, encryption_addon, sizeof(encryption_addon));
 ...
 while((c=fgetc(fp))!=EOF){
	...
	//
	  ...
	  //  for (x = 0; i< 25; i++){
	    ...
	    //printf("attempted decrpytion produces = %s\n", debuf);
